...
export default function PortfolioViewPage() {
  ...
  const layoutWrap = (content) => {
    if (layout === 'sidebar') {
      const showSidebar = modules.bio || (modules.contact && CONTACT_FIELDS.some((f) => contact[f.key]))
      return (
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <main className={`flex-1 min-w-0 ${!showSidebar ? 'lg:mx-auto lg:max-w-4xl' : ''}`}>
              {modules.bio && heroSection()}
              {projectsSection()}
              {skillsSection()}
              {workSection()}
              {blogSection()}
              {modules.contact && contactSection()}
            </main>
          </div>
        </div>
      )
